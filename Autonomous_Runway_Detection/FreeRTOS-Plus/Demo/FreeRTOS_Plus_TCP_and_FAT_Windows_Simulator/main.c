...
/* Handle of the task that runs the Edge Detection. */
static TaskHandle_t xEdgeDetectionTaskHandle = NULL;

/* Handle of the task that runs the RSA Encryption. */
static TaskHandle_t xRsaEncryptionTaskHandle = NULL;

/* Handle of the task that runs the TCP Send data. */
static TaskHandle_t xTcpSendDataTaskHandle = NULL;

/* Handle of the queue required for sending and receiving the output of edge detection */
QueueHandle_t edge_encrypt_queue_handle;

/* Handle of the queue required for sending and receiving the output of Encryption */
QueueHandle_t encrypt_tcp_queue_handle;
/*-----------------------------------------------------------*/
...
int main(void)
{
	...
	if ((config_SERVER_ADDR0 == 0) || (config_SERVER_PORT == 0))
	{
		printf("Configure the Server address and port in tcp_client.h. Exiting the program\n");
		exit(-1);
	}
...
#ifdef ENABLE_RSA
	edge_encrypt_queue_handle = xQueueCreate(QUEUE_SIZE, sizeof(pack_data_t));
#endif

#ifdef ENABLE_TCP
	encrypt_tcp_queue_handle = xQueueCreate(QUEUE_SIZE, sizeof(pack_encrypt_data_t));
#endif

#ifdef ENABLE_CANNY
	xTaskCreate(vEdgeDetectionTask, "EdgeDetection", 1000, NULL, EDGE_DETECTION_TASK_PRIORITY, &xEdgeDetectionTaskHandle);
#endif

#ifdef ENABLE_RSA
	xTaskCreate(vRsaEncryptionTask, "RsaEncryption", 1000, NULL, RSA_ENCRYPTION_TASK_PRIORITY, &xRsaEncryptionTaskHandle);
#endif
...
void vApplicationIPNetworkEventHook(eIPCallbackEvent_t eNetworkEvent)
{
	...
	if (eNetworkEvent == eNetworkUp)
	{
		...
		{
			xTaskCreate(vTcpSendDataTask, "TcpSendData", 1000, NULL, TCP_CLIENT_TASK_PRIORITY, &xTcpSendDataTaskHandle);
