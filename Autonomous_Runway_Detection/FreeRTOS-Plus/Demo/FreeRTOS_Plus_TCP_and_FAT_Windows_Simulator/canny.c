                  ...
                  bitmap_info_header_t *bitmapInfoHeader)
{
    ...
    for(size_t i=0; i< (size_t)bitmapInfoHeader->height; i++){
	    for(size_t j=0; j< (size_t)bitmapInfoHeader->width; j++){
              ...
              const pixel_t *data)
{
    ...
    unsigned char c;
    for(size_t i=0; i < (size_t)bmp_ih->height; i++) {
	    for(size_t j=0; j < (size_t)bmp_ih->width; j++) {
...
void vEdgeDetectionTask(void* par)
{
	...
	char input_file_name_path[MAX_CHAR_FILE_PATH];
	char output_file_name_path[MAX_CHAR_FILE_PATH];
	TickType_t tick;

	printf("Starting Edge Detection Task\n");
	tick = xTaskGetTickCount();	
	for (int index = 1; index <= NUM_OF_FILES; index++)
	{
		snprintf(input_file_name_path, MAX_CHAR_FILE_PATH, "%s\\%s%d).bmp", INPUT_FILE_DIR, FILE_NAME_PATTERN, index);
		snprintf(output_file_name_path, MAX_CHAR_FILE_PATH, "%s\\%s%d).bmp", OUTPUT_FILE_DIR, FILE_NAME_PATTERN, index);

#ifdef ENABLE_MY_LOGGING
		printf("Input File - %s and Edge detected output file - %s\n", input_file_name_path, output_file_name_path);
#endif
		const pixel_t *in_bitmap_data = load_bmp(input_file_name_path, &ih);
		...
		if (save_bmp(output_file_name_path, &ih, out_bitmap_data)) {
		...
		memcpy(pack_data.out_data, out_bitmap_data, ih.bmp_bytesz);
		pack_data.out_data[ih.bmp_bytesz] = DATA_SIZE_DELIMITER;

		while (pdTRUE != xQueueSend(edge_encrypt_queue_handle, &pack_data, 0))
		{
	#ifdef ENABLE_MY_LOGGING
			printf("Edge detection Queue Sending failed, retrying\n");
	#endif
			// Wait for receive to complete
			vTaskDelay(portTICK_PERIOD_MS * 500);
		}
		file_num++;
		printf("Sending to queue with image size - %d\n", ih.bmp_bytesz + 1);
	#ifdef ENABLE_DEMO_LOGGING
		printf("Sending file %d data - %d, %d, %d, %d\n", file_num, pack_data.out_data[DEMO_DATA_INDEX_1], pack_data.out_data[DEMO_DATA_INDEX_2],
			pack_data.out_data[DEMO_DATA_INDEX_3], pack_data.out_data[DEMO_DATA_INDEX_4]);
	#endif
	...
	printf("Done with %d files\n", file_num);
	printf("Time taken to detect edges for %d files - %dms\n", NUM_OF_FILES, tick);
	printf("Exiting Edge Detection Task\n");
}